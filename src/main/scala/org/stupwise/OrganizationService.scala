...
class OrganizationService[F[_]: Async: Parallel](client: GitHubClient[F]) extends Http4sDsl[F] {
  ...
  private def getAllContributors(orgName: String): F[List[ContributorResult]] = {
    for {
      ...
      contributors <- repositories.parFlatTraverse(rep => getContributors(orgName, rep.name))
    } yield contributors
...
object OrganizationService {
  ...
  def apply[F[_]: Async: Parallel](client: GitHubClient[F]): OrganizationService[F] = {
